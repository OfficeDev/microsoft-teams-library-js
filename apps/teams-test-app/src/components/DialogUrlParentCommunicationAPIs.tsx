...
import { dialog, IAppWindow, ParentAppWindow } from '@microsoft/teams-js';
...
const DialogUrlParentCommunicationAPIs = (): ReactElement => {
  const childWindowRef = React.useRef<IAppWindow | null>(null);

  const CheckDialogUrlParentCommunicationCapability = (): ReactElement =>
    ApiWithoutInput({
      ...
      onClick: async () => {
        if (dialog.url.parentCommunication.isSupported()) {
          return 'Dialog Url parent communication module is supported';
        } else {
          return 'Dialog Url parent communication module is not supported';
        }
      },
    });
  ...
  return (
    <ModuleWrapper title="Dialog.Url.ParentCommunication">
      <CheckDialogUrlParentCommunicationCapability />
      <SendMessageToChild />
      <SendMessageToParent />
