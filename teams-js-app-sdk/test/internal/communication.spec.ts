...
describe('communication', () => {
  ...
  it('shouldProcessMessage calls validateOrigin', () => {
    ...
    const messageOriginObject = new URL(messageOrigin);
    const result = communication.shouldProcessMessage(null, messageOrigin);
    expect(utils.validateOrigin).toBeCalled();
    expect(utils.validateOrigin).toBeCalledWith(messageOriginObject);
    expect(result).toBe(utils.validateOrigin(messageOriginObject));
  });
});