...
import { isThenable } from '../../src/internal/utils';
...
describe('files', () => {
  ...
  //       expect(() => files.openDownloadFolder(null)).toThrowError(
  //         'This call is only allowed in following contexts: ["content"]. Current context: "settings"',
  //       );
  //     });
  ...
  //     //   const maybePromise = files.openDownloadFolder(null, callback);

  //     //   //this is true, if overloads are working this should be false
  //     //   expect(isThenable(maybePromise)).toBe(true);
  ...
  //         expect(err).toBeFalsy();
  //       };
  //       files.openDownloadFolder(null, callback).finally(() => {
  //         expect(callback).toHaveBeenCalled();
  //       });
  //       const openDownloadFolderMessage = utils.findMessageByFunc('files.openDownloadFolder');
  //       expect(openDownloadFolderMessage).not.toBeNull();
