...
describe('utils', () => {
  ...
  it('fallbackValidateOrigin returns true if origin is in teams pre-known allowlist', () => {
    const messageOrigin = new URL('https://teams.microsoft.com');
    const messageOriginHost = messageOrigin.host;
    const result = fallbackValidateOrigin(messageOriginHost);
    expect(result).toBe(true);
  });
  it('fallbackValidateOrigin returns true if origin for subdomains in teams pre-known allowlist', () => {
    const messageOrigin = new URL('https://subdomain.teams.microsoft.com');
    const messageOriginHost = messageOrigin.host;
    const result = fallbackValidateOrigin(messageOriginHost);
    expect(result).toBe(true);
  });
  it('fallbackValidateOrigin returns false if origin is not in teams pre-known allowlist', () => {
    const messageOrigin = new URL('https://badorigin.com');
    const messageOriginHost = messageOrigin.host;
    const result = fallbackValidateOrigin(messageOriginHost);
    expect(result).toBe(false);
  });
  it('fallbackValidateOrigin returns false if origin is not an exact match in teams pre-known allowlist', () => {
    const messageOrigin = new URL('https://team.microsoft.com');
    const messageOriginHost = messageOrigin.host;
    const result = fallbackValidateOrigin(messageOriginHost);
    expect(result).toBe(false);
  });
  it('validateAdditionalOrigins returns true if origin is valid origin supplied by user ', () => {
    const messageOrigin = new URL('https://testorigin.com');
    GlobalVars.additionalValidOrigins = [messageOrigin.origin];
    const messageOriginHost = messageOrigin.host;
    const result = validateAdditionalOrigins(messageOriginHost);
    expect(result).toBe(true);
  });
  it('validateAdditionalOrigins returns false if origin is not supplied by user', () => {
    const messageOrigin = new URL('https://badorigin.com');
  ...
  it('validateAdditionalOrigins returns true if origin for subdomains is in the user supplied list', () => {
  ...
  it('validateAdditionalOrigins returns false if origin for subdomains is not in the user supplied list', () => {
    const messageOrigin = new URL('https://subdomain.badorigin.com');
    GlobalVars.additionalValidOrigins = ['https://*.testorigin.com'];
    const messageOriginHost = messageOrigin.host;
    const result = validateAdditionalOrigins(messageOriginHost);
  ...
  it('fallbackValidateOrigin returns false if the port number of valid origin is not in teams pre-known allowlist', () => {
    const messageOrigin = new URL('https://local.teams.live.com:4000');
    const messageOriginHost = messageOrigin.host;
    const result = fallbackValidateOrigin(messageOriginHost);
    expect(result).toBe(false);
  });
  it('validateAdditionalOrigins returns false if the port number of valid origin is not in the user supplied list', () => {
  ...
  it('fallbackValidateOrigin returns true if the port number of valid origin is in teams pre-known allowlist', () => {
  ...
  it('fallbackValidateOrigin returns false if origin has extra appended', () => {
    const messageOrigin = new URL('https://teams.microsoft.com.evil.com');
    const messageOriginHost = messageOrigin.host;
    const result = fallbackValidateOrigin(messageOriginHost);
    expect(result).toBe(false);
  });
  it('fallbackValidateOrigin returns false if first end of origin is not matched valid subdomains in teams pre-known allowlist', () => {
  ...
  it('fallbackValidateOrigin returns false if origin for subdomains does not match in teams pre-known allowlist', () => {
    const messageOrigin = new URL('https://a.b.sharepoint.com');
    const messageOriginHost = messageOrigin.host;
    const result = fallbackValidateOrigin(messageOriginHost);
    expect(result).toBe(false);
  });
  it('validateAdditionalOrigin returns false if origin for subdomains does not match in the user supplied list', () => {
