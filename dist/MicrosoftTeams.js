...
(function (microsoftTeams) {
    ...
    (function (authentication) {
        ...
        function openAuthenticationWindow(url, width, height) {
            ...
            if (hostClientType === hostClientTypes.desktop) {
                ...
                var messageId = sendMessageRequest(parentWindow, "authentication.allowWindowOpen", [link.href]);
                callbacks[messageId] = function (allowed) {
                    if (allowed) {
                ...
                var top_1 = (typeof currentWindow.screenTop !== "undefined") ? currentWindow.screenTop : currentWindow.screenY;
                left += (currentWindow.outerWidth / 2) - (width / 2);
                top_1 += (currentWindow.outerHeight / 2) - (height / 2);
                // Open the window with a desired set of standard browser features
                childWindow = currentWindow.open(link.href, "_blank", "toolbar=no, location=yes, status=no, menubar=no, top=" + top_1 + ", left=" + left + ", width=" + width + ", height=" + height);
            }
