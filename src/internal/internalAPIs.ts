...
function updateRelationships(messageSource: Window, messageOrigin: string): void {
  ...
  const noParentOrMessageIsFromParent = !GlobalVars.parentWindow || GlobalVars.parentWindow.closed || messageSource === GlobalVars.parentWindow;
  const isParentMessage = !GlobalVars.isFramelessWindow && noParentOrMessageIsFromParent;
  const isChildMessage = !GlobalVars.childWindow || GlobalVars.childWindow.closed || messageSource === GlobalVars.childWindow;
