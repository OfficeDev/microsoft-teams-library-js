...
export function initializeCommunication(callback: Function, validMessageOrigins: string[] | undefined): void {
  ...
  CommunicationPrivate.handlers['themeChange'] = handleThemeChange;
  CommunicationPrivate.handlers['fullScreenChange'] = handleFullScreenChange;
  CommunicationPrivate.handlers['backButtonPress'] = handleBackButtonPress;
  CommunicationPrivate.handlers['load'] = handleLoad;
  CommunicationPrivate.handlers['beforeUnload'] = handleBeforeUnload;
  CommunicationPrivate.handlers['changeSettings'] = handleChangeSettings;
  CommunicationPrivate.handlers['startConversation'] = handleStartConversation;
  CommunicationPrivate.handlers['closeConversation'] = handleCloseConversation;
  CommunicationPrivate.handlers['appButtonClick'] = handleAppButtonClick;
  CommunicationPrivate.handlers['appButtonHoverEnter'] = handleAppButtonHoverEnter;
  CommunicationPrivate.handlers['appButtonHoverLeave'] = handleAppButtonHoverLeave;
...
export function registerHandler(name: string, handler: Function): void {
  CommunicationPrivate.handlers[name] = handler;
}

export function removeHandler(name: string): void {
  delete CommunicationPrivate.handlers[name];
}
...
function handleParentMessage(evt: DOMMessageEvent): void {
  if ('id' in evt.data && typeof evt.data.id === 'number') {
    ...
    const handler = CommunicationPrivate.handlers[message.func];
    if (handler) {
...
function handleChildMessage(evt: DOMMessageEvent): void {
  if ('id' in evt.data && 'func' in evt.data) {
    ...
    const message = evt.data as MessageRequest;
    const handler = message.func ? CommunicationPrivate.handlers[message.func] : null;
    if (handler) {
